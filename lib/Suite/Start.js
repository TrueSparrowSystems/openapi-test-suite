...
class Start {
  ...
  constructor() {
    ...
    oThis.stringIncorrectValuesArray = StringParamValueSet.incorrectValues();
    oThis.integerIncorrectValuesArray = IntegerParamValueSet.incorrectValues();
  }
  ...
  async testAllApis() {
    ...
    for (const path in oThis.allPaths) {
      ...
      for (const method in currentPath) {
        const stringParamCorrectValue = StringParamValueSet.correctValue(),
          integerParamCorrectValue = IntegerParamValueSet.correctValue();

        // TODO - Ajinkya - stringParamValueSet.incorrectValues(), etc should be called here.
        const stringCombinationsArray = oThis.stringIncorrectValuesArray.concat(stringParamCorrectValue),
          integerCombinationsArray = oThis.integerIncorrectValuesArray.concat(integerParamCorrectValue);
        ...
        for (let index = 0; index < combinationsArray.length; index++) {
          ...
          for (const param in paramToTypeMap) {
            ...
            if (paramToTypeMap[param] == 'string' && paramValue == stringParamCorrectValue.value && allParamsCorrect) {
              allParamsCorrect = true;
            } else if (
              paramToTypeMap[param] == 'number' &&
              paramValue == integerParamCorrectValue.value &&
              allParamsCorrect
            ) {
              allParamsCorrect = true;
            } else {
