...
class Start {
  ...
  async testCustomApis() {
    ...
    logger.log('--------------------------- STARTED TESTING CUSTOM APIS --------------------------- ');
    ...
    logger.log('--------------------------- ENDED TESTING CUSTOM APIS ---------------------------');
  }
   ...
   * @returns {*}
   */
  getParamValueFromResp(reference, responseArray) {
    const pathArray = reference.split('.');
    const respIndex = pathArray[0].split('[')[1].split(']')[0];
    let response = responseArray[respIndex];
